...
DWORD WINAPI ThreadFunc(LPVOID lpThreadParameter)
{
	while (1)
	{
		...
		if (bytesWritten == 0)
		{
			...
			printf("thread ID: %d ,error code: %d --WriteFile(mainPipeServer, pipeThreadClientName2,...) returned 0 bytes as written!! \n",p,err);
			Sleep(10000);
			return 0;
		}
		...
		{
			int err = GetLastError();
			printf("thread ID: %d ,error code: %d--WriteFile(mainPipeServer, pipeClientThreadName2...) returned 0 bytes as written!!\n",p,err);
			Sleep(10000);
			return 0;
		}
		...
		{
			int err = GetLastError();
			printf("thread ID: %d ,error code: %d--WriteFile(mainPipeServer, pipeCreatedName2...) returned 0 bytes as written!!\n",p,err);
			Sleep(10000);
			return 0;
		}
		...
		{
			int err = GetLastError();
			printf("thread ID: %d ,error code: %d --WriteFile(mainPipeServer, askForDiscoName2...) returned 0 bytes as written!!\n",p,err);
			Sleep(100000);
			return 0;
		}
		////
		...
		{
			int err = GetLastError();
			printf("thread ID: %d ,error code: %d - CreateNamedPipe(pipeThreadClient2,...) returned INVALID_HANDLE_VALUE %d\n", p,err);
			Sleep(10000);
			return 0;
		}
		...
		{
			int err = GetLastError();
			printf("thread ID: %d ,error code: %d --CreateFile(pipeClientThreadName2,...) returned INVALID_HANDLE_VALUE %d \n",p, err);
			Sleep(100000);
			return 0;
		}
		printf("all connections done in Thread ID :%d !!\n",p);
		...
		while (1)
		{
			...
			{
				int err = GetLastError();
				printf("thread ID: %d ,error code: %d -- couldn't read the client command from pipeClientThread2 \n",p,err);
				Sleep(10000);
				return 0;
			}
			if (intt == 1) {
				...
				{
					int err = GetLastError();
					printf("thread ID: %d ,error code: %d --couldn't read the size of the PATH from pipeClientThread2\n",p,err);
					Sleep(10000);
					return 0;
				}
				...
				if (res == 0)
				{
					printf("thread ID: %d ,error code: %d -- couldn't read the PATH  from pipeClientThread2\n");
					Sleep(10000);
					return 0;
				}
				buffer[sizee - 1] = NULL;
				...
				for (int i = 0; i < dex; i++)
				{
					...
					{
						int err = GetLastError();
						printf("thread ID : %d, error code : %d -- couldn't write to pipeThreadClient2 the size of name of the directory/file found\n",p,err);
						return 0;
					}
					bytesWritten = 0;
					...
					{
						int err = GetLastError();
						printf("thread ID : %d, error code : %d -- couldn't write to pipeThreadClient2 the file/directory name\n",p,err);
						return 0;
					}
				}
				...
				{
					int err = GetLastError();
					printf("thread ID : %d, error code : %d -- couldn't write to pipeThreadClient2 the size of 'Done'\n",p,err);
					return 0;
				}
				bytesWritten = 0;
				...
				{
					int err = GetLastError();
					printf("thread ID : %d, error code : %d -- couldn't write to pipeThreadClient2 the word 'Done',which signals the end of the enumeration\n",p,err);
					return 0;
				}
			}
			else if (intt == 2) {
				...
				{
					int err = GetLastError();
					printf("thread ID: %d ,error code: %d --couldn't read the size of the FULL PATH from pipeClientThread2\n",p,err);
					Sleep(100000000);
					return 0;
				}
				...
				{
					int err = GetLastError();
					printf("thread ID: %d ,error code: %d --couldn't read the FULL PATH from pipeClientThread2\n",p,err);
					Sleep(100000000);
					return 0;
				}
				bufu[sizee - 1] = NULL;
				...
				{
					int err = GetLastError();
					printf("thread ID: %d ,error code: %d -- couldn't CreateFile with the FULL PATH:%s\n", p,err,bufu);
					Sleep(10000);
					return 0;
				}
				strcpy_s(bufu2, bufu);
				while (1) {
					...
					{
						int err = GetLastError();
						printf("thread ID: %d ,error code: %d -- failed to read from %s \n",p,err,bufu2);
						Sleep(10000);
						return 0;
					}
					DWORD bytesWritten = 0;
					...
					{
						int err = GetLastError();
						printf("thread ID : %d, error code : %d -- couldn't write to pipeThreadClient2 contents from %s\n",p,err,bufu2);
						Sleep(10000);
						return 0;
					}
					if (bytesRead != 300)
				...
				printf("All connections closed with the client\n");
				break;
			}
		}
	}
